...
/***Constants***/
#define MIN_THROTTLE 000L
#define MAX_THROTTLE 200L  //870
#define MAX_CONTROL_VALUE 1024L
...
void motor_init(void)
{
	...
	//(0xA0) Enables OC1A and OC1B pin and clears on compare match
	TCCR1A= (1<<7 | 0<<6 | 1<<5 | 0<<4 | 0<<3 | 0<<2 | 0<<1 | 0<<0); 
	//(0x12) Input noise canceller deactivated,  falling edge of input capture pin (ignored), clk/1 (8Mhz)
	TCCR1B= (0<<7 | 0<<6 | 0<<5 | 1<<4 | 0<<3 | 0<<2 | 0<<1 | 1<<0);
	...
	//Enable ADC, start ADC, set in free-running mode, disable interrupt, divide XTAL by 64 (125kHz)
...
void motor_set_speed(uint16_t value)
{
	if(enable == true)
	{
		...
		uint32_t difference = MAX_THROTTLE - MIN_THROTTLE;
		// Set the output compare register (compares TCNT1)
		OCR1A = ((uint32_t)value*(difference))/(MAX_CONTROL_VALUE) + MIN_THROTTLE;
	}
}
