...
{
	// Set baud rate (9600). FIX (it should be 51)
	UBRRH = 0;
	
	// Platform is 1 if board, 2 if remote
	if(platform == 1)
	{
	...
	// Enable TX and RX, enable rx interrupts (tx interrupts enabled when data loaded into tx buffer)
